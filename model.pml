...
chan user_channel[USER_NUM] = [1] of {mtype};
chan user_channel_uid[USER_NUM] = [1] of {mtype, byte};
...
proctype server() {
	...
	byte place; 
		...
		:: purchases != PLACE_NUM && purchases != USER_NUM && len(server_channel) == 0 ->
			skip;
	od;
}
...
/* TODO У любых двух пользователей не может быть одинаковых мест; */
